...
// Candidate details for internal hiring
...
var requirementint RequirementInt
...
func InternalMatchRequirement(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < len(externalcandidates); i++ {
		if externalcandidates[i].ExternalHiring() == true {
			matched = append(matched, externalcandidates[i])
		}
	}
...
func (extcand *Candidate) ExternalHiring() bool {
